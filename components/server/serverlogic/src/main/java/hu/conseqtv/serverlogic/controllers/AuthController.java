...
public class AuthController {
    ...
    public ResponseEntity<?> authenticateUser(@Valid @RequestBody LoginRequest loginRequest) {
        logRepository.save(new Log("Started user authentication for " + loginRequest.getUsername(), LogLevel.INFO));
        Authentication authentication = authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(loginRequest.getUsername(), loginRequest.getPassword()));
        logRepository.save(new Log("Successfully authenticated " + loginRequest.getUsername() + ", generating token (not logging further)", LogLevel.INFO));
    ...
    public ResponseEntity<?> modifyUser(@Valid @RequestBody ModificationRequest modificationRequest) {
        logRepository.save(new Log("Trying to modify user " + modificationRequest.getUsername(), LogLevel.INFO));
        Optional<User> user;
        if(!(user = userRepository.findByUsername(modificationRequest.getUsername())).isPresent()) {
            logRepository.save(new Log("User " + modificationRequest.getUsername() + " does not exist!", LogLevel.ERROR));
            return ResponseEntity.badRequest().body(new MessageResponse("User does not exist!"));
        }

        if(modificationRequest.getPassword() != null) {
            logRepository.save(new Log("Updating password of " + modificationRequest.getUsername(), LogLevel.INFO));
            user.get().setPassword(encoder.encode(modificationRequest.getPassword()));
        }
        if(modificationRequest.getRole() != null) {
            logRepository.save(new Log("Trying to update role of user " + modificationRequest.getUsername(), LogLevel.INFO));
            Role role;
            ...
            logRepository.save(new Log("Updated role of user " + modificationRequest.getUsername(), LogLevel.INFO));
        }
        userRepository.save(user.get());
        logRepository.save(new Log("User " + modificationRequest.getUsername() + " modified successfully", LogLevel.INFO));
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
        logRepository.save(new Log("Trying to register user " + signUpRequest.getUsername(), LogLevel.INFO));
        if (userRepository.existsByUsername(signUpRequest.getUsername())) {
            logRepository.save(new Log("User " + signUpRequest.getUsername() + " already exists", LogLevel.ERROR));
        ...
        logRepository.save(new Log("Successfully registered user " + signUpRequest.getUsername(), LogLevel.INFO));
